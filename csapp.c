 ...
 * 注意到，当中断产生后，rio_readn 函数会自动下一轮循环，因为nread == 0
 * 没有缓冲区可言，直接写入了目标地址
 * 返回值是实际读取的字节数，若出错返回-1
 */
/* $begin rio_readn */
 ...
 * 在写入时若被中断，nwritten = 0，也会自动下一轮循环
 * 除非出错返回-1，否则n个字节一定会被全部写入，故一直返回n
 */
 ...
 * 对 Unix read() 函数的包装，增加了内部的缓冲区，每次从缓冲区传回
 * min(n, rio_cnt) 个字节给用户，当缓冲区中内容为空时，再调用 read() 填满
 */
 ...
 * 调用了rio_read() 替代 read()，一个一个字符读，读到\n
 * 就跳出循环，结束函数，未读到数据时，也是跳出循环
 * readlineb 和 readnb都带缓冲区 相互兼容，且因缓冲区在rio_t中，故线程安全
 */
